            ...
            <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
                <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                    <form @submit.prevent="submit" class="p-4 sm:p-8 space-y-6">
                        ...
                        <!-- Gambar -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Gambar (Kosongkan jika tidak ingin diubah)</label>
                            
                            <div v-if="post.image_path && !form.delete_image" class="mb-4">
                                <p class="text-xs text-gray-500 mb-2">Gambar saat ini:</p>
                                <div class="relative inline-block">
                                    <img :src="'/storage/' + post.image_path" class="h-40 object-cover rounded-md border border-gray-200" alt="Current Image">
                                    <button type="button" @click="form.delete_image = true" class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 shadow-md hover:bg-red-600 transition" title="Hapus Gambar">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                    </button>
                                </div>
                            </div>

                            <div v-if="form.delete_image" class="mb-4 p-3 bg-red-50 border border-red-200 text-red-600 rounded-md text-sm flex items-center justify-between">
                                <span>Gambar akan dihapus saat disimpan.</span>
                                <button type="button" @click="form.delete_image = false" class="text-red-700 underline font-medium">Batal Hapus</button>
                            </div>
